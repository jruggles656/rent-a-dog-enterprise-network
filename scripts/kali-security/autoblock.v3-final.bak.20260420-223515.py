...
# Wipe-alert tuning
WIPE_HEARTBEAT_EVERY = 0     # 0 = disabled (was 100; user found it noisy)
...
def _ts():
    """UTC timestamp for log files (universal forensics)."""
    return datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S")


def _ts_local():
    """Human-readable local time for Telegram messages.
    Falls back to UTC if zoneinfo not available."""
...
def reconcile_pf_with_blocked(blocked: dict, dry: bool, wipe_stats: dict) -> int:
    ...
    elif WIPE_HEARTBEAT_EVERY > 0 and n % WIPE_HEARTBEAT_EVERY == 0:
